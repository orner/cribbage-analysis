...
pypy3 tools/gen_opponent_model.py      # regenerate opponent_model.py     (~90 s)
...
- **Second iteration of the opponent model.** `opponent_model.py` was generated
  by opponents who decide using the *uniform* model, so it is one refinement
  step, not a fixed point. Re-run `tools/gen_opponent_model.py` with
  `opponent="policy"` in the sampling call and see whether the tables move. I
  expect little, since the first step shifted decisions by only ~8%, but that is
